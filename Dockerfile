FROM --platform=linux/amd64 nginx:1.26.0
...
RUN cp /opt/opentelemetry-webserver-sdk/WebServerModule/Nginx/1.26.0/ngx_http_opentelemetry_module.so /etc/nginx/modules/ && \
    echo "load_module /etc/nginx/modules/ngx_http_opentelemetry_module.so;\n$(cat /etc/nginx/nginx.conf)" > /etc/nginx/nginx.conf
